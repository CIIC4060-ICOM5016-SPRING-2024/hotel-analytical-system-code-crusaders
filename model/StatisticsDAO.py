    ...
    def get_MostProfitMonth(self):
        result = Database().querySelectFrom(
            ...
            total_reservations_by_month_year as
                (
                    select  *,
                            count(month) as total_reservations
                        from extract_month_and_year
                    group by chainID, month, year, name
                    order by chainID, total_reservations desc
                )
            ...
            select name, year, month, total_reservations 
                ...
                (
                    select  *, 
                            row_number() over
                            (
                                partition by chainID
                                order by total_reservations desc
                            ) as highest
                    from total_reservations_by_month_year
                ) result
                where highest <= 3;
            """, ()
        )
